export async function adminCreateUser(student_id: string, name: string, password: string, class_id?: string | null): Promise<{ id?: string; error?: string }> {
  const r = await fetch('/api/users', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ student_id, name, password, class_id }),
  })
