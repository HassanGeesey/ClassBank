...
export function THead({ children, className }: TableProps) {
  return (
    <thead className={cn('border-b border-border bg-bg-elevated', className)}>
      {children}
    </thead>
  )
}
...
export function Th({ children, className }: TableProps) {
  return (
    <th className={cn('px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-secondary', className)}>
      {children}
    </th>
  )
}
